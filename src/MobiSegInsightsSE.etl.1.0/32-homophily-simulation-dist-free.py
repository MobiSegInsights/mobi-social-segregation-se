...
class HomophilyDistanceFreeSim:
    def __init__(self):
        self.gdf_stops = None
        self.gdf_pois = None
        self.df_home = None
    ...
    def stop_data_loader(self, test=False):
        ...
        tree = KDTree(self.gdf_pois[["y", "x"]], metric="euclidean")
    ...
    def poi2home(self, row):
        ...
        if len(osm_idx) > 0:
            ...
            df.loc[:, "deso_s"] = self.gdf_pois.loc[osm_idx, 'deso_s'].values
            if len(df) > 0:
                ...
                if df.loc[:, "tag_cat"].sum() > 0:
                    ...
                    for cat in (1, 2, 3, 4):
                        ...
                        if len(df2sample) > 0:
                            deso_pool = random.choices(df2sample['deso_s'].values, k=100)
                            break
                else:
                    deso_pool = []
            else:
            ...
            if len(deso_pool) > 0:
                ds_str = str(dict(Counter(deso_pool)))
            else:
                ds_str = ''
            return pd.Series(dict(deso_sd=ds_str))
